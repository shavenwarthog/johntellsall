...
import numpy
import sys, os, getopt, random, hashlib
...
##                    print '\033[91m Start with Sudo / Admin rights \033[0m'
                ...
                try:
                    ...
                    print 'Error:'
                    print 'For help try:test.py --help'
                    sys.exit(2)

                for opt, arg in opts:
                    if opt in ("-h", "--help"):
                        print 'Helpfile: -h or --help'
                        print '-h, --help = Helpfile'
                        print '-c <camID> = Camera ID'
                        sys.exit()
                    elif opt in ("-c", "--camID"):
                        camID = int(arg)
                    elif opt in ("-i", "--ifile"):
		...
		if inputfile != '':
			self.writer = None
			self.capture = cv.CaptureFromFile(inputfile)
	...
	def run(self):
		...
		while True:
			...
			cv.Threshold( grey_image, grey_image, 240, 255, cv.CV_THRESH_BINARY )
                        ...
                        ### Hier kommen die Abdeckungsdreiecke rein!!! ###
...
#######hierwar ich testen was die vorherige funktion zeichnet und bounding_box_list ausgeben
...
##testpunkt danach eventuell der fehler mit den vielen eintraegen
			...
			#print len(trimmed_center_points)
			#print trimmed_center_points
#######ab hier abändern!!!
			for target in trimmed_center_points:
				...
				# errechnet von jedem neuen target den abstand zu jedem alten target in x und y richtung
				for entity in last_frame_entity_list:
					
					entity_coords= entity[3]
					delta_x = entity_coords[0] - target[0] #Abweichung x
					delta_y = entity_coords[1] - target[1] #Abweichung y
			
					distance = sqrt( pow(delta_x,2) + pow( delta_y,2) ) #keine ahnung
					entity_distance_dict[ distance ] = entity #testen was das macht
				...
				for distance in distance_list:
					...
					# Don't consider entities that are already claimed:   ###hier muss ein fehler sein in der if abfrage bzw danach beim schreiben
					...
					this_frame_entity_list.append( nearest_possible_entity ) #eventuell if abfrage ob es es schon gibt
			...
			for entity in last_frame_entity_list:
				...
				if frame_t0 - last_time_seen > entity_ttl:
					...
					for entity in this_frame_entity_list:
                                                if entity[0] == entity_last[0]:# and entity[2] > entity_last[2]: #und last time seen größer
                                                        this_frame_entity_list.remove(entity)
                                            
					
                                        this_frame_entity_list.append( entity_last )
			
                        #print len(this_frame_entity_list)
                        #print this_frame_entity_list
			# For next frame:
			self.targets = this_frame_entity_list
